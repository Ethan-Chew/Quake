    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        ...
        case kDescriptionElementContent:
            ...
            if self.seekDescription {
                    ...
                    // search the entire string for the user's country, and extract that last part of that string
                
                var userCountry = userDefaults.string(forKey: "User Country")
                
                let searchedRange = NSRange(0..<self.currentParsedCharacterData.utf16.count)
                let regExpression = try! NSRegularExpression(pattern: userCountry ?? "", options: [])
                ...
                if let match = regExpression.firstMatch(in: self.currentParsedCharacterData, options: [], range: searchedRange) {
                    ...
                    print("missing \(userCountry) in \(kDescriptionElementContent) element")
                }
